...
export const DispatchGoalCompletedDiscordEventJobHandler: JobHandler<Payload> = {
  options: {
    backoff: {
      type: 'exponential',
      delay: 30_000
  ...
  async execute(payload) {
    ...
    for (const { groupId } of memberships) {
      const dispatchResult = await dispatchDiscordBotEvent(DiscordBotEventType.GOAL_COMPLETED, {
        groupId,
        player,
        metric: goal.metric,
